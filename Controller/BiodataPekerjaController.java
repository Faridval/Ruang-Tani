...
public class BiodataPekerjaController implements Initializable {

    @FXML
    private TextField Pengalaman_Button;
    ...
    private void saveData(ActionEvent event) {
        String sqlUpdate = "UPDATE pekerja SET Nama = ?, Alamat = ?, no_KTP = ?, no_hp = ?, riwayat_pekerjaan = ? WHERE ID_Pekerja = ?";
        String sqlInsert = "INSERT INTO pekerja (Nama, Alamat, no_KTP, no_hp, riwayat_pekerjaan, ID_Pekerja) VALUES (?, ?, ?, ?, ?, ?)";
        ...
        try {
            if (isInputValid()) {
                ...
                // Validasi: cek apakah ID_Pekerja sudah ada
                String checkSql = "SELECT COUNT(*) FROM pekerja WHERE ID_Pekerja = ?";
                ...
                prepare.setString(5, Pengalaman_Button.getText());
    ...
    private boolean isInputValid() {
        ...
        if (Pengalaman_Button.getText().isEmpty()) {
            showAlert(Alert.AlertType.ERROR, "Validasi", "Pengalaman/Riwayat Pekerjaan harus diisi.");
            return false;
        }
